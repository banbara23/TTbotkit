...
// カメラのチェック
controller.hears(['カメラチェック'], 'ambient', function(bot, message) {
  child_process.exec('fswebcam -d /dev/video0 --list-controls', function(error, stdout, stderr) {
    if (error) {
      bot.reply(message, 'エラー');
      bot.reply(message, stderr);
    } else {
